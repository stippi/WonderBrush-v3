...
TransformBox::Init()
{
	fView->Invalidate(Bounds().InsetByCopy(-INSET, -INSET));
}

// Cleanup
void
TransformBox::Cleanup()
{
	fView->Invalidate(Bounds().InsetByCopy(-INSET, -INSET));
}
...
// MouseDown
void
TransformBox::MouseDown(BPoint where, uint32 buttons, uint32 clicks)
{
...
// MouseMoved
void
TransformBox::MouseMoved(BPoint where, uint32 transit,
