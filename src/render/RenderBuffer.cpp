...
{
	CopyTo(reinterpret_cast<uint8*>(bitmap->Bits()), bitmap->BytesPerRow(),
		bitmap->Bounds(), area);
}
...
{
	CopyTo(buffer->Bits(), buffer->BytesPerRow(), buffer->Bounds(), area);
}

// CopyTo
void
RenderBuffer::CopyTo(uint8* dst, uint32 dstBPR,
	BRect dstBounds, BRect area) const
{
...
// BlendTo
void
RenderBuffer::BlendTo(BBitmap* bitmap, BRect area) const
{
	BlendTo(reinterpret_cast<uint8*>(bitmap->Bits()), bitmap->BytesPerRow(),
		bitmap->Bounds(), area);
}
...
{
	BlendTo(buffer->Bits(), buffer->BytesPerRow(), buffer->Bounds(), area);
}

// BlendTo
void
RenderBuffer::BlendTo(uint8* dst, uint32 dstBPR,
	BRect dstBounds, BRect area) const
{
	// make sure we don't copy out of bounds
	area = area & dstBounds;
