...
// SaveItem class
SaveItem::SaveItem(const char* name, BMessage* message, uint32 exportMode)
	:
...
// SavePanel class
SavePanel::SavePanel(const char* name, BMessenger* target,
		entry_ref* startDirectory, uint32 nodeFlavors,
		bool allowMultipleSelection, BMessage* message,
		BRefFilter* filter, bool modal, bool hideWhenDone)
	:
	...
	fExportMode(EXPORT_MODE_BITMAP)
{
	...
	window->SetTitle(B_TRANSLATE("Save document"));
	...
	fFormatMF = new BMenuField(rect, "format popup", B_TRANSLATE("Format"),
		fFormatM, true,	B_FOLLOW_LEFT | B_FOLLOW_BOTTOM,
	...
	fSettingsB = new BButton(rect, "settings", 
		B_TRANSLATE("Settings"B_UTF8_ELLIPSIS), new BMessage(MSG_SETTINGS),
...
SavePanel::SendMessage(const BMessenger* messenger, BMessage* message)
{
	...
	if (message != NULL && fFormatM->IsEnabled())
...
SavePanel::SetExportMode(bool exportMode)
{
	...
	if (exportMode) {
		...
		_EnableSettings();
		helper << B_TRANSLATE_CONTEXT("Export document", "Dialog title");
	} else {
		fExportMode = ExportMode();
		...
		helper << B_TRANSLATE_CONTEXT("Save document", "Dialog title");
	}

	window->SetTitle(helper);
	window->Unlock();
}
...
SavePanel::_BuildMenu()
{
	...
	fNativeMI = new SaveItem("WonderBrush", 
...
//	fFormatM->AddItem(fIconMimeAttrMI);
	...
	// pick the native item in the list
	fNativeMI->SetMarked(true);
}
